...
`--idl-root` discovers `.djinni` files recursively, parses each canonical source once,
and resolves the complete graph before generating bindings. Use either `--idl`
(the existing single-file mode) or `--idl-root`:

```sh
./src/run-assume-built --idl-root ./djinni \
  --idl-project ./djinni-project.yaml \
  --multiple-inheritance true \
  --cpp-out ./shared/public \
  --kotlin-out ./bridging/android \
  --java-package io.openmobilemaps.mapscore.shared \
  --jni-out ./bridging/jni \
  --objc-out ./bridging/ios \
  --objcpp-out ./bridging/ios \
  --wasm-out ./bridging/wasm \
  --ts-out ./bridging/ts \
  --yaml-out ./bridging/yaml \
  --ts-import-prefix @djinni/maps-core/
```

Keep the usual C++ namespaces, Objective-C type prefixes, include prefixes and KMP
settings required by your project. Directory mode does **not** enable multiple
inheritance implicitly. Parent bridges see descendants in every discovered file;
no inheritance umbrella or per-file SDK loop is needed.

#### Source identity and layout

For `map/layers/line/line.djinni`:

| Setting | Directory-mode result |
|---|---|
| Java/Kotlin package | `<java-package>.map.layers.line` |
| Java/Kotlin/JNI directories | `<output-root>/map/layers/line` |
| TypeScript file | `<ts-out>/map/layers/line/line.ts` |
| TypeScript import with the example prefix | `@djinni/maps-core/map/layers/line/line` |
| C++, Objective-C, Objective-C++, WASM, YAML directories | Existing configured flat layout |
| KMP directories, package and bridge naming | Existing explicit KMP settings |

Relative TypeScript imports are computed between defining files. Enum imports
remain usable in generated constant expressions. If a module aggregate and a
type's static interface would have the same name, the module aggregate uses a
`Module_statics` suffix (for example, `LineModule_statics`). JNI/WASM headers get
relative C++ include paths when the C++ output is enabled and no explicit bridge
C++ include prefix was supplied. Include the JNI output root in your compiler's
header search path. Explicit imports outside the root are still followed; those
sources use the base package and output directories.

#### Optional project configuration

Paths and globs are relative to `--idl-root`; rules apply in listed order.
The project file has only three optional keys:
...
rules:
  - path: web/**
    disable: [java, kotlin, jni, objc, objcpp, kmp]
  - path: compatibility/**
    java-package: io.example.legacy
externs:
  ../old-generated/core-types.yaml: map/core.djinni
```

`disable` accepts `cpp`, `java`, `kotlin`, `jni`, `objc`, `objcpp`, `wasm`, `ts`,
`yaml`, and `kmp`. Rules may also override `java-package` and `ts-module`.
Common generator options remain CLI options. Unknown settings are errors.
`--idl-exclude <glob>` can be repeated in addition to the configuration's excludes.
Exclusions affect discovery; an explicitly imported source is still included.
Directory mode rejects `@flag` directives: move common flags to the invocation and
package/module exceptions to project rules. Single-file `@flag` handling is unchanged.

An extern matching a local YAML output path belongs to its source, including when
an old generated YAML file exists. A missing extern can also match a unique source
filename stem or type name (including `--yaml-prefix`). Ambiguous matches are errors.
`externs` explicitly maps an arbitrary YAML path to its defining source file; neither
file order nor a pre-generated YAML file is required. Existing YAML outside these
owned paths remains external; a duplicate internal/external type requires an explicit
ownership mapping. Local references may use the configured YAML name prefix.
`--yaml-out-file` still writes one combined YAML file, with each type's own metadata.

#### Output ownership

Directory mode buffers generated content and detects collisions across **all**
generators before replacing output. The default manifest is
`<idl-root>/.djinni-manifest`; use `--idl-manifest <file>` for a different location
or separate invocations over the same root. Keep this file between runs, and normally
ignore it in version control. It records absolute output paths and content hashes.

Unchanged files keep their modification times. Removed definitions and disabled
outputs remove only previously owned, unmodified files. Handwritten files are not
adopted or deleted. On the first run, existing files bearing Djinni's autogenerated
preamble can be adopted; other existing files must have identical content or generation
fails. Editing an owned generated file causes a refusal to overwrite or delete it.
Errors during parsing, resolution, collision checks or generation leave existing
output untouched and exit nonzero. Replacements use atomic file renames; this is
not an atomic transaction across the entire filesystem if the process or machine
fails during the commit. Concurrent runs sharing output directories are unsupported.
Generated content is buffered in memory, so peak memory grows with total output size.

The SDK script only needs to build or locate Djinni and issue this invocation.
