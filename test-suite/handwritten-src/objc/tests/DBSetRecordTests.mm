...
- (DBSetRecord *)getObjcSetRecord
{
    ...
    DBSetRecord *objcSetRecord = [DBSetRecord setRecordWithSset:set iset:[NSSet set]];
...
- (void)testCppToObjc
{
    ...
    XCTAssertEqual([objcSetRecord.sset count], (NSUInteger)3, @"Set length 3 expected, actual: %lu", (unsigned long)[objcSetRecord.sset count]);
    XCTAssert([objcSetRecord.sset containsObject:@"StringA"], @"\"StringA\" expected but does not exist");
    XCTAssert([objcSetRecord.sset containsObject:@"StringB"], @"\"StringB\" expected but does not exist");
    XCTAssert([objcSetRecord.sset containsObject:@"StringC"], @"\"StringC\" expected but does not exist");
}
