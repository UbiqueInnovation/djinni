...
class KotlinKmpGenerator(spec: Spec) extends Generator(spec) {
  ...
  private def iosActualTypename(td: TypeDecl): String = {
    ...
    if (iosModule.isEmpty) objcTypeWithProtocol else s"$iosModule.$objcTypeWithProtocol"
  }
  ...
  private def platformType(tm: MExpr, isAndroid: Boolean): String = {
    ...
    def externBase(e: MExtern): String = {
      if (isAndroid) {
        ...
        def objcSystemModule(header: String): Option[String] = {
        ...
        objcSystemModule(e.objc.header) match {
          ...
          case None => if (resolvedModule.isEmpty) objcType else s"$resolvedModule.$objcType"
        }
      }
    }
