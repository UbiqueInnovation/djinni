...
class KotlinKmpGenerator(spec: Spec) extends Generator(spec) {
  ...
  private def writeKotlinFile(folder: File, fileName: String, origin: String, f: IndentWriter => Unit): Unit = {
    createFile(folder, fileName, (w: IndentWriter) => {
      w.wl("// AUTOGENERATED FILE - DO NOT MODIFY!")
      w.wl("// This file was generated by Djinni from " + origin)
      w.wl
      spec.kotlinKmpPackage.foreach(s => w.wl(s"package $s").wl)
      f(w)
    })
  }
  ...
  private def generateRecord(td: TypeDecl, r: Record): Unit = {
    ...
    spec.kotlinKmpIosOutFolder.foreach(folder => {
      ...
      writeKotlinFile(folder, s"$name.kt", origin, w => {
        ...
        w.w(s"actual class $name$typeParams public constructor(")
        if (r.fields.nonEmpty) {
          w.wl
          w.increase()
          for (f <- r.fields) {
  ...
  private def generateEnum(td: TypeDecl, e: Enum): Unit = {
    ...
    spec.kotlinKmpIosOutFolder.foreach(folder => {
      ...
      writeKotlinFile(folder, s"$name.kt", origin, w => {
        ...
        w.increase()
        w.wl(s"internal fun fromPlatform(value: $actualType): $name {")
        w.increase()
        w.wl("val raw = value.toLong()")
        ...
        w.wl("}")
        w.wl
        w.wl(s"internal fun $name.asPlatform(): $actualType = rawValue")
      })
    })
  }
  ...
  private def generateInterfaceDecl(td: TypeDecl, i: Interface): Unit = {
    ...
    spec.kotlinKmpAndroidOutFolder.foreach(folder => {
      writeKotlinFile(folder, s"$name.kt", origin, w => {
        ...
        if (classKind) {
          w.wl(s"actual class $name$typeParams public constructor(")
          w.increase()
          ...
          w.wl
          w.wl(s"internal fun $name$typeParams.asPlatform(): $platformType = when (this) {")
    ...
    def externBase(e: MExtern): String = {
      if (isAndroid) {
        ...
        objcSystemModule(e.objc.header) match {
          ...
          case None => if (iosModule.isEmpty) objcType else s"$iosModule.$objcType"
        }
      }
    }
  ...
  private def kmpType(tm: MExpr): String = {
    ...
    def f(tm: MExpr): String = {
      tm.base match {
        ...
        case e: MExtern => kmpBridgePrefix + idJava.ty(e.name) + (if (e.java.generic) args(tm) else "")
        case p: MProtobuf => p.name
        case o =>
          val base = o match {
            case p: MPrimitive => p.kName
            ...
            case e: MExtern => kmpBridgePrefix + idJava.ty(e.name)
            case p: MProtobuf => p.name
            case p: MParam => idJava.typeParam(p.name)
            case MVoid => "Void"
          }
